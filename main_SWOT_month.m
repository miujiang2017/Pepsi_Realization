...
%                       sigma_dA_obs(i,j) =sqrt(sum(dAerr_obs(i,j,:).^2)/10000);
    ...
    if use_Q_rlz == 1
        ...
        sigma_Q_obs(idx_r,idx_c) = abs(Q(idx_r,idx_c)-Q_withunc_1(idx_r,idx_c));
    ...
    med_corr_unc{1} = median(corr_unc)
    med_NSE_unc{1} = median(NSE_unc)
    med_rRMSE_unc{1} =median(rRMSE_unc)
    med_corr_obs{1} = median(corr_obs)
    med_NSE_obs{1} = median(NSE_obs)
    med_rRMSE_obs{1} =median(rRMSE_obs)
...
%                 end
            ...
            [Phi,Q_st] = build_Phi_SWOT_month(nR,L_center_pos,c,D,tau,Q,pct(k)*0.01,state_ep,choice_Q);
...
dates = datevec(this);
idx_month = dates(2);
            %idx_month = ceil((i+1)/30);
            xnn1(:,i) =  Phi{idx_month} *  xn1n1;
            Pnn1{i} = (Phi{idx_month} * Pn1n1 * Phi{idx_month}') + Q_st{idx_month};
            ...
            if ~isempty(zn)
                ...
                if use_Q_obs == 1
                    ...
                    [H_Q,z_Q,R_Q] = build_H_obs_SWOT_Q(L(grch),dA_cell,i,Q,state_ep,Q_withunc,Q_res_unc,R_choice,use_Q_rlz,sigma_Q_withunc);
                    H= [H;H_Q];
                    zn = [zn;z_Q];
                    size_R = size(R,1);
                    size_RQ = size(R_Q,1);
...
dates = datevec(this);
idx_month = dates(2);
                xnn1(:,i) =  Phi{idx_month} *  xnn{i-1};
                Pnn1{i} = (Phi{idx_month} * Pnn{i-1} * Phi{idx_month}') + Q_st{idx_month};
            ...
            %% combine xnn
            [Q_est,Qest_weight,Qest_arith,Qest_med] = combine_xnn_SWOT(xnn,Pnn,nR,nt,state_ep,Q);
