...
Q_withunc_syn = Q_withunc; % synthetic one
% realization contains Nan for 10k rlz
[idx_r,idx_c]= find(isnan(med_Q_obs)==1);

med_Q_obs(idx_r,idx_c) = Q_withunc_syn(idx_r,idx_c);
...
for i = 1:length(grch1)
    for j = 1:nt
        [idx1] = find(isnan(Q_obs(i,j,:))==1);
        Q_obs_10k(i,j,idx1) = med_Q_obs(i,j);
    end
end
%存10k个的Q_rlz
save(['Q_rlz_10k',num2str(m),'.mat'],'Q_obs_10k')
...
nR = size(Q,1);
cho =1;%size(choice,1)
Qest_rlz = cell(10000,25);
for n_rlz = 1:10000
    Q_withunc_2(grch1,:) = Q_obs_10k(:,:,n_rlz);
    diff_idx = setdiff(grch,grch1);
    Q_withunc_2(diff_idx,:) = Q_withunc_syn(diff_idx,:);
    sigma_Q_withunc = [];
    %if use_Q_rlz == 1
    ...
    %     end
    Q_res_unc =inf;% std(Q(:,2:end)-Q_withunc(:,2:end),1,2);
    ...
    for i = 1:nR
        if ~isnan(find(i==grch1))
            ...
            for j = 1:length(reserve{i})
                dA_cell{i,reserve{i}(j)}=dA_obs(ns,reserve{i}(j),n_rlz);
    ...
    %% KF
    k = 1;%1:length(pct) % percentage for process noise
    state_ep = 22;
    %% begin values
    xn1n1 = [];
    for i = 1:state_ep
    ...
    Pnn1={};
    xnn1=[];
    NSE_med_rlz{n_rlz,1} = NSE_med{1};
    NSE_med_obs{n_rlz,1} = NSE_obs;
    ...
    Qest_rlz(n_rlz,:) = [Q_est,Qest_weight{1},Qest_arith{1},Qest_med{1}];
    %     corr_rlz = [corr_est,corr_weight{1},corr_arith{1},corr_med{1}];
    %     NSE_rlz = [NSE_est,NSE_weight{1},NSE_arith{1},NSE_med{1}];
    %     rRMSE_rlz = [rRMSE_est,rRMSE_weight{1},rRMSE_arith{1},rRMSE_med{1}];
    %             for n = 1:state_ep-1
    ...
    %     save(['corr_Qrlz',num2str(m),'.mat'],'corr_rlz')
    %     save(['NSE_Qrlz',num2str(m),'.mat'],'NSE_rlz')
    %     save(['rRMSE_Qrlz',num2str(m),'.mat'],'rRMSE_rlz')
    %     else
...
save(['Qest_Qrlz',num2str(m),'.mat'],'Qest_rlz')
%end
