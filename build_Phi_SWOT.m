function [Phi_st,Q_st] = build_Phi_SWOT(nR,L_center_pos,c,D,tau,Q,coeff,state_ep,choice_Q)
...
%            
%             if ep1>=0
%                % R_crs_self2(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,g:end-ep1)',Q(j,k+1:end)');
% R_crs_self(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,1:end-ep1)'-mean(Q(i,:)),Q(j,ep1+1:end)'-mean(Q(j,:)));
%             else
%                %  R_crs_self2(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,g:end)',Q(j,k+1:end+ep1)');
%                R_crs_self(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,-ep1+1:end)'-mean(Q(i,:)),Q(j,1:end+ep1)'-mean(Q(j,:)));
%             end
% %             
%              if ep2>=0
%                %  R_auto_self2(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,g:end-ep2)',Q(j,k:end)');
%                 R_auto_self(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,1:end-ep2)'-mean(Q(i,:)),Q(j,ep2+1:end)'-mean(Q(j,:)));
%              else
%                %  R_auto_self2(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,g:end)',Q(j,k:end+ep2)');
%                  R_auto_self(j+nR*(k-1),i+nR*(g-1)) = corr(Q(i,-ep2+1:end)'-mean(Q(i,:)),Q(j,1:end+ep2)'-mean(Q(j,:)));
%              end
%         end
%     end
%     end
...
% R_auto_mat = R_auto_self;
% R_crs_mat = R_crs_self;
